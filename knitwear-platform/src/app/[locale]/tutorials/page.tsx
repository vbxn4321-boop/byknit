...
function VectorEditorDemo({ locale }: { locale: string }) {
    ...
    return (
        <div className="w-full relative bg-[#FAF6F0] rounded-2xl border border-[#EFE7DC] shadow-soft overflow-hidden p-4 md:p-6" style={{ minHeight: '410px' }}>
...
// 2. 차트 변환기 벡터 시연 플레이어
// ==========================================
function VectorConverterDemo({ locale }: { locale: string }) {
    ...
    return (
        <div className="w-full relative bg-[#FAF6F0] rounded-2xl border border-[#EFE7DC] shadow-soft overflow-hidden p-4 md:p-6" style={{ minHeight: '430px' }}>
            ...
            <div className="mb-4 bg-[#FFF9EB] border border-[#FFE7B3] rounded-xl p-2.5 flex items-center gap-2 text-stone-600 select-none shadow-sm animate-in fade-in duration-300">
                <AlertTriangle className="w-4 h-4 text-amber-500 flex-shrink-0" />
                <span className="text-[10px] md:text-xs font-black text-amber-800 leading-tight">
                    {locale === 'ko' 
                        ? '⚠️ 검증 필요: AI가 생성한 도안은 구조적 오류가 있을 수 있습니다. 실제 제작 전에 반드시 검토해주세요.' 
                        : '⚠️ Verification: AI patterns may contain structural errors. Check before knitting.'}
                </span>
            </div>
...
// ==========================================
function VectorTranslatorDemo({ locale }: { locale: string }) {
    const [mouse, setMouse] = useState({ x: 92, y: 88 });
    const [mouseClicked, setMouseClicked] = useState(false);
    const [ripple, setRipple] = useState<{ x: number; y: number; show: boolean }>({ x: 0, y: 0, show: false });
    const [toast, setToast] = useState<string | null>(null);
...
function VectorCalculatorDemo({ locale }: { locale: string }) {
    const [mouse, setMouse] = useState({ x: 92, y: 88 });
    const [mouseClicked, setMouseClicked] = useState(false);
    const [ripple, setRipple] = useState<{ x: number; y: number; show: boolean }>({ x: 0, y: 0, show: false });
    const [toast, setToast] = useState<string | null>(null);
