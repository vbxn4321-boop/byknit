-- Final Definitive Fix for Marketplace Algorithm
-- This script cleans up ALL previous attempts and installs the guaranteed working version.

-- 1. DROP EVERYTHING (Clean Slate) --------------------------------
-- Drop all possible variations of previous functions to prevent conflicts
...
-- 2. CREATE THE FINAL FUNCTION (get_mk_patterns) -------------------
-- Exact match with frontend parameters (11 params)
CREATE OR REPLACE FUNCTION public.get_mk_patterns(
    ...
    p_format TEXT DEFAULT NULL,       -- Explicitly included
...
LANGUAGE plpgsql
SECURITY DEFINER -- Run with privileges of the creator to ensure access
AS $$
DECLARE
    v_interest_categories TEXT[];
BEGIN
    -- Get User Interests (Top 3 categories based on likes)
    ...
    WHERE
        ...
        -- Array filters (checks for overlap)
...
-- 3. PERMISSIONS ---------------------------------------------------
-- Explicitly grant execute permission to all roles
