-- Sync View Counts in Marketplace
-- Previously, get_mk_patterns calculated views from 'pattern_views' table join.
-- But increment_pattern_view_count only updates 'patterns.view_count' column.
-- This caused list view (8) to lag behind modal view (25).
-- FIX: Update get_mk_patterns to use p.view_count column directly.

DROP FUNCTION IF EXISTS public.get_mk_patterns(uuid, int, int, text, text, text, text[], text[], text, boolean, text, double precision);

-- Note: We use the signature with p_random_seed
CREATE OR REPLACE FUNCTION public.get_mk_patterns(
    ...
    p_random_seed DOUBLE PRECISION DEFAULT 0
)
RETURNS TABLE (
    id UUID,
    title JSONB,
...
BEGIN
    ...
    WITH pattern_stats AS (
        SELECT
            ...
            -- NO JOIN with pattern_views here. Use column instead.
    ...
    SELECT
        ...
        p.designer_id,
        p.created_at,
        p.view_count::BIGINT as view_count, -- Cast to BIGINT to match return type
        ...
        (
            CASE
                ...
                WHEN p_sort = 'popular' THEN
                    (ps.likes * 10) + p.view_count + (ps.review_count * 20) -- Use p.view_count here too
                ELSE -- 'recommended'
                    (GREATEST(0, 60 - EXTRACT(DAY FROM (now() - p.created_at))) / 60.0 * 40) +
                    (LN(1 + p.view_count + (ps.likes * 5) + (ps.review_count * 10)) * 10) + -- Use p.view_count here too
