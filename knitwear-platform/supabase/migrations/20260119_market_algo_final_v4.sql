-- Final Robust Fix (V4)
-- This script handles potential data type mismatches (e.g., if images are JSONB vs TEXT[]) by explicit casting.
...
RETURNS TABLE (
    ...
    is_free BOOLEAN,
    status TEXT,
    images JSONB,  -- Changed to JSONB to be safe regardless of underlying type
    category TEXT,
    difficulty TEXT,
    designer_id UUID,
...
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, extensions -- Setting search path for security
AS $$
DECLARE
...
BEGIN
    ...
    SELECT
        ...
        p.is_free,
        p.status::text,
        -- Robust image handling: Convert to JSONB whether it's text[] or jsonb
        to_jsonb(p.images),
        p.category::text,
        p.difficulty::text,
        p.designer_id,
    ...
    WHERE
        ...
        -- Search filter with explicit casting check
