-- 1. ADD NEW COLUMNS TO public.patterns TABLE
ALTER TABLE public.patterns ADD COLUMN IF NOT EXISTS is_official BOOLEAN DEFAULT false;
ALTER TABLE public.patterns ADD COLUMN IF NOT EXISTS item_type TEXT DEFAULT 'digital' CHECK (item_type IN ('digital', 'physical'));
ALTER TABLE public.patterns ADD COLUMN IF NOT EXISTS purchase_url TEXT;

-- 2. DROP OLD get_mk_patterns RPC FUNCTION
DROP FUNCTION IF EXISTS public.get_mk_patterns(uuid, int, int, text, text, text, text[], text[], text, boolean, text, double precision);

-- 3. CREATE UPDATED get_mk_patterns RPC FUNCTION WITH NEW COLUMNS IN THE RETURN TABLE
...
BEGIN
    ...
    SELECT
        ...
        p.designer_id,
        p.created_at,
        p.view_count::BIGINT as view_count,
        ...
        (
            CASE
                ...
                WHEN p_sort = 'popular' THEN
                    (ps.likes * 10) + p.view_count + (ps.review_count * 20)
                ELSE -- 'recommended'
                    (GREATEST(0, 60 - EXTRACT(DAY FROM (now() - p.created_at))) / 60.0 * 40) +
                    (LN(1 + p.view_count + (ps.likes * 5) + (ps.review_count * 10)) * 10) +
