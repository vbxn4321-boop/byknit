-- MASTER FIX: Schema + Function (Final Solution)
-- 1. FIX THE TABLE SCHEMA FIRST (Missing columns caused the crash)
DO $$
BEGIN
    -- Add price_krw if missing
...
-- 2. DROP ALL PREVIOUS FUNCTION VERSIONS
...
BEGIN
    ...
    SELECT
        ...
        p.title,
        p.description,
        -- Safe casting for possibly new columns
        p.price_usd::numeric,
        p.price_krw::numeric,
        p.is_free,
        p.status::text,
